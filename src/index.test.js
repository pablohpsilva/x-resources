...
import { applyQueryStringsOnURL, getHttpClientLibraryMethod, prepareBaseURL, createResources, resources } from '.'
...
test('fetchSpecificUser from resource', async () => {
    const response = await simpleResource.fetchSpecificUser({ username, id }).catch(error => {
        throw error
    })
...
test('createResources should return the resource', () => {
    ...
    const action = { method, url: '/user/:username' }
